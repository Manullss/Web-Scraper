...
def extractor(driver,m):    #se encarga de abrir, navergar y extraer los datos de la pagina 
    ...
    while m != 0:
        ...
        ms=[]              #datos mes 
        di=[]              #datos dia   
        extra=[None] * 31       #guarda los datos finales para ser enviados a Dataframe
        ...
        for dia in range(4):             #Buscamos los primeros elementos de la pagina y los extraemos
            ...
            if dia == 3:        #Continuamos al siguiente mes
                e=me[m-2]
                driver.click("input#datepicker")
                driver.click('li[data-view="month prev"]')
                driver.sleep(0.5)
        ...
        while len(extraidos) < 31:   #Si los datos extraidos son menores de 31 añadimos datos vacios asta llegar a 31
            extraidos.append(None)

        while len(di) < 31:     #si los datos son menores a 32 añadimos datos vacios asta llegar a 31
            di.append(None)

        while len(ms) < 31:     #si los datos son menores a 32 añadimos datos vacios asta llegar a 31
            ms.append(None)
        
        for i in reversed(dias):        #Empezamos por 31 asta llegar a 1
            if ms[i-1] == m:            #Guardamos los datos extraidos deacuerdo al dìa que han aparecido
                for d in di:
                    if d == i :
                        extra[i-1] = extraidos[d-1]

        if len(ms) >= 32 and len(di) >= 32:
            ms.pop(31)
            di.pop(31)
        #------------------------------------------------------------------------

        print("3- ",len(extra),"-",extra)
...
def excel_extraidos(extraidos1,m,dias):  #Añadimos los Datos extraidos al archivo excel por medio de la libreria pandas
    mes=["ENERO","FEBRERO","MARZO","ABRIL","MAYO","JUNIO","JULIO","AGOSTO","SEPTIEMBRE","OCTUBRE","NOVIEMBRE","DICIEMBRE"]
    
    df=pd.DataFrame({"Dia":dias,f"{mes[m-1]}":extraidos1}).set_index("Dia") 
    print("-----------")
